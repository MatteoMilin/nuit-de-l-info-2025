...
export default function Snake() {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      const isOpposite =
        direction[0] === -newDir[0] && direction[1] === -newDir[1];
      if (!isOpposite) {
  ...
  const styles: Record<string, React.CSSProperties> = {
    ...
    board: {
      background: '#000',
      boxShadow:
        '0 0 30px rgba(0, 255, 0, 0.6), inset 0 0 30px rgba(0, 255, 0, 0.05)',
      borderRadius: 0,
    },
    head: {
      background: '#0f0',
  ...
  return (
    <div
      className='flex flex-col items-center justify-center h-screen w-screen overflow-hidden bg-black p-2'
          ...
          <div key={row} className='flex'>
            {Array.from({ length: SIZE }).map((_, col) => {
              ...
              return (
                <div
                  key={col}
                  className={`w-6 h-6 border ${cellStyles[type]}`}
                  style={styles[type]}
                />
              );
            })}
